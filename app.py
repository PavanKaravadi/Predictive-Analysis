...
def load_model():
    try:
        ...
        return make_pipeline(StandardScaler(), RandomForestRegressor(random_state=42))
...
if st.button("Predict for Next 30 Days"):
    ...
    st.metric("Total Predicted Cases (30 days)", f"{result_df['Predicted Daily Cases'].sum():,.0f}")
